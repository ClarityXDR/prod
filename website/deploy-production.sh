...
if [ "$healthy" = true ]; then
    ...
    # Clean up old images
    print_message $YELLOW "Cleaning up old ClarityXDR images only..."
    # Only remove ClarityXDR-specific images
    docker image prune -f --filter "label=clarityxdr" 2>/dev/null || true
    docker rmi $(docker images --filter "reference=*clarityxdr*" --filter "dangling=true" -q) 2>/dev/null || true
